...
class Veml6030:
    """
    Gets data from the veml6030 light sensor over i2c on Raspberry PI Pico
    ...
    def init_device(self):
        print("Init VEML6030 device")
        self.i2cbus.writeto_mem(self.veml6030_address,self.ALS_CONF,b'00')
        print("Config VEML6030 device")
        self.i2cbus.writeto_mem(self.veml6030_address,self.ALS_WL,self.CONFIG)
        time.sleep(0.020)       # Wait 20ms for i2c bus

    def measure_light(self):
        """Outputs the brighness in lumens"""
        print("Measuring Light")
        raw_bits = self.i2cbus.readfrom_mem(self.I2C_ADDR, self.ALS, 2)
        print(f"raw_bits: {raw_bits}")
        
        return int.from_bytes(raw_bits, "big") * self.LUX_TO_BIT_MULTIPLIER
